...
> {
  ...
  readonly description =
    'List the retained history versions of ONE document, newest first — the per-document alternative to a workspace snapshot, so recovering one clobbered note does not roll anything else back. Each entry carries a history_path, which is what get_document_history_content and rollback_document take. op says what produced the version (update, delete, format, clean). Versions are captured on a timer (ten minutes by default), NOT on every write, so a change made and undone within one interval was never recorded — the version you find will hold the damage rather than what came before it. SiYuan also prunes history on its retention setting, so this covers recent loss, not last month\'s; anything older, or anything that has to survive an interval, needs a snapshot. exhaustive: false means the scan hit its cap and older versions may exist — raise max_timestamps to look further back.';
